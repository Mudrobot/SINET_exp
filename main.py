...
def main():
    ...
    spatial_transform_train = ST.Compose([
                ST.Scale((args.height, args.width), interpolation=3), # 图片缩放
                ST.RandomHorizontalFlip(), # 随机水平翻转
                ST.ToTensor(), #  转换为Tensor
                ST.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]), #  归一化
                ST.RandomErasing()]) # 随机擦除
    ...
    trainloader = DataLoader(
        VideoDataset(
            ...
            spatial_transform=spatial_transform_train,
            temporal_transform=temporal_transform_train),
        sampler=RandomIdentitySampler(dataset.train, num_instances=args.num_instances), # num_instances=4,默认每个ID取4个图像
    ...
    model = init_model(
                name=args.arch, # sinet
                num_classes = dataset.num_train_pids, # 最终分类数量
                losses=args.losses,
                seq_len=args.seq_len)

    print_time("Model Size w/o Classifier: {:.5f}M".format( # 计算模型大小，numel返回tensor p中的元素数量
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=args.stepsize, gamma=args.gamma) # 在达到milestone时，学习率分别*gamma
    ...
    if use_gpu:
        model = nn.DataParallel(model).cuda() # DataParallel 实现了在多个GPU上并行处理数据
...
def train(epoch, model, criterions, optimizer, trainloader, use_gpu):
    ...
    for batch_idx, (vids, pids, camid) in enumerate(trainloader): # trainbatch 默认大小是32
        pd.set_postfix({'Acc': '{:>7.2%}'.format(batch_corrects.avg), })
        pd.update(1)
        if (pids-pids[0]).sum() == 0:
            continue
        # vids shape batchsz*C*num_instances(sampled t)*H*W
        if use_gpu:
            vids = vids.cuda()
            pids = pids.cuda()
