...
class iLIDSVID(object):
    ...
    def __init__(self, root, split_id=0):
        ...
        num_imgs_per_tracklet = num_imgs_train + num_imgs_query + num_imgs_gallery
        min_num = np.min(num_imgs_per_tracklet) # 最小帧数
        max_num = np.max(num_imgs_per_tracklet) # 最大帧数
        avg_num = np.mean(num_imgs_per_tracklet)# 平均帧数

        num_total_pids = num_train_pids + num_query_pids # 总共目标ID数
        num_total_tracklets = num_train_tracklets + num_query_tracklets + num_gallery_tracklets # 总共的轨迹数量
